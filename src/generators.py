...
def filter_by_currency(transactions: List[Dict[str, Any]], currency_code: str) -> Generator[Any, None, None]:
    """Генератор возвращает транзакции, где валюта операции соответствует заданному коду (например, RUB)"""
    if transactions == []:
        yield "Передан пустой список"
    else:
        for i_dict in transactions:
            if currency_code in i_dict["operationAmount"]["currency"].get("code"):
                yield i_dict
